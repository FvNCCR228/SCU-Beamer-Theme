...
\mailscu{⟨email⟩}                   % 封面邮箱（自带前导空白，换行后忽略）
\date{\today}
```

- `\and` — 分隔不同作者或机构
- `\inst{⟨tag⟩}` — 为作者/机构/邮箱设置归属标签（数字、字母、符号均可）
- `\mailscu{⟨email⟩}` — 设置封面邮箱。自带前导空白（无需手动加 `~`），换行后自动忽略空白
...
\institute{%
  ...
  \vspace*{-6pt} \and
  \inst{a} ~\textit{zhangsan@scu.edu.cn}
  ~\inst{b} ~\textit{lisi@scu.edu.cn}%
}
```
...
4. **生成代码** — 按照模板生成 beamer-scu 代码（见 templates.md）
