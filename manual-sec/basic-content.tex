...
\subsection{字体}
\begin{frame}{Info.}
	\textbf{本小节将介绍字体的设置, 包括字号、字体属性、强调字体与添加线及标点符号.}
	\begin{multicols}{2}
		\begin{itemize}
...
\begin{frame}{字号}
	...
	\begin{columns}[onlytextwidth]
		...
		\begin{column}{.2\textwidth}
			Beamer全局默认字号为 \alert{normalsize}, 一般情况不必修改默认字号.\\[1ex]
			局部调整建议使用左侧中 \cmd{footnotesize} 到 \cmd{Large} 间的字号.\\[3ex]
...
\begin{frame}{字体属性}
	...
	\begin{table}[h]
		...
		\begin{tabular}{>{\bfseries\color{PrimaryC}}m{.05\textwidth}R{.26\textwidth}m{.16\textwidth}m{.12\textwidth}R{.26\textwidth}}
		  & \alert{\textbf{名称}} & \alert{\textbf{调用命令}} & \alert{\textbf{声明命令}} & \alert{\textbf{中英文显示效果}} \\[.4ex]
			\toprule
		  ...
		  & 中等 Medium             & \cmd{textmd}\marg{text} & \cmd{mdseries} & Master Ma 形翼马掌门          \\
			...
			& Bold Italic       & \multicolumn{2}{c|}{{\bfseries\itshape Master Ma 形翼马掌门}} & \multirow{2}{=}{\centering\alert{\textbf{常用场景}}} \\
...
\begin{frame}[fragile]{添加线及标点符号}
	...
	标点符号:
	\begin{itemize}
		\item 建议使用半角标点符号, 后键入一个空格.
		\item 双引号由两对左单引号、右单引号构成: `` ''. 左单引号在键盘上ESC键下方.
		\item 建议使用实心的句号, 只要书写的是自然科学类文章.
	\end{itemize}
