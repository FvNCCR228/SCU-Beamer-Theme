...
\begin{frame}{\pkg{beamethemescu} 宏包参数说明(1/5)}{ColorDisplay, BlockDisplay}
	...
	\begin{table}[h]
		...
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.25\textwidth}p{0.68\textwidth}}
			...
			(D) \texttt{JXred} & 四川大学锦锈红主题. \textcolor{scured}{\rule[-.1ex]{1.2em}{1.8ex}} (CMYK 12, 92, 95, 20)\\
			\texttt{BSblue} & 四川大学宝石蓝主题. \textcolor{scublue}{\rule[-.1ex]{1.2em}{1.8ex}} (CMYK 100, 60, 00, 15)\\
...
\begin{frame}{\pkg{beamethemescu} 宏包参数说明(2/5)}{CodeDisplay, MintedStyle, FontTheme}
	\begin{table}[h]
		\centering
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.25\textwidth}p{0.68\textwidth}}
			v1.1a \alert{\ttfamily CodeDisplay} & \textbf{代码高亮显示设置}\\
			(D) \texttt{listing} & listing排版引擎.\\
			\texttt{minted} & minted排版引擎. (需Python环境, 设置前请阅读手册)\\
			\midrule

			v1.3a (C) \alert{\ttfamily MintedStyle} & \textbf{minted样式设置(需优先设置 \texttt{\alert{CodeDisplay}=minted})}\\
