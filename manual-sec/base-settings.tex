...
\begin{frame}{\pkg{beamethemescu} 宏包已废除参数}
	\begin{table}[h]
		...
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.3\textwidth}p{0.15\textwidth}p{0.45\textwidth}}
			...
			~\alert{\ttfamily Miniframes}\texttt{ = negate} & v2.1b & 由 \texttt{Miniframes}\texttt{ = none} 替代.\\
			~\alert{\ttfamily NavigationTool}\texttt{ = negate} & v2.1b & 由 \texttt{NavigationTool}\texttt{ = none} 替代.\\
			~\alert{\ttfamily Background}\texttt{ = true} & v2.1c & 由 \texttt{Background}\texttt{ = SCU-Full} 替代.\\
			~\alert{\ttfamily Background}\texttt{ = false} & v2.1c & 由 \texttt{Background}\texttt{ = SCU-Lite} 替代.\\
			~\alert{\ttfamily CodeDisplay}\texttt{ = \Arg{value}} & v2.1d & 由 \alert{\ttfamily CodeTheme}\texttt{ = \Arg{value}} 替代.\\
		\end{tabular}
	\end{table}
\end{frame}
