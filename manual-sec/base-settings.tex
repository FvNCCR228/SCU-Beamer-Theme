...
\begin{frame}{\pkg{beamethemescu} 宏包参数说明(1/5)}{ColorDisplay, BlockDisplay}
	...
	\begin{table}[h]
		...
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.25\textwidth}p{0.68\textwidth}}
			...
			v1.3a \alert{\texttt{ColorDisplay}} & \textbf{颜色主题设置}\label{back:ColorDisplay} \scugoto{goto:ColorDisplay}{PREV}\\
...
\begin{frame}{\pkg{beamethemescu} 宏包参数说明(2/5)}{CodeDisplay, MintedStyle, FontTheme}
	\begin{table}[h]
		...
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.25\textwidth}p{0.68\textwidth}}
			...
			v2.1c \alert{\texttt{FontTheme}} & \textbf{字体主题设置}\\
...
\begin{frame}{\pkg{beamethemescu} 宏包参数说明(3/5)}{MathFont, Miniframes, NavigationTool}
	...
	\begin{table}[h]
		\centering
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.25\textwidth}p{0.68\textwidth}}
			v1.1a (C) \alert{\texttt{MathFont}} & \textbf{数学字体设置}\\
			...
			v1.3d \alert{\texttt{NavigationTool}} & \textbf{页脚导航工具栏设置}\label{back:NavigationTool} \scugoto{goto:NavigationTool}{PREV}\\
...
\begin{frame}{\pkg{beamethemescu} 宏包参数说明(4/5)}{LogoWidth, Background, VI, LanguageMode}
	\begin{table}[h]
		\centering
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.25\textwidth}p{0.68\textwidth}}
			v2.1c \alert{\texttt{LogoWidth}} & \textbf{页脚Logo区宽度设置}\\
			(D) \texttt{48pt} & 默认宽度.\\
			\Arg{dimen} & 自定义宽度值, 如 \texttt{36pt}, \texttt{1cm}.\\
			\midrule

			v2.1c \alert{\texttt{Background}} & \textbf{背景显示设置}\\
			...
			v2.1c \alert{\texttt{VI}} & \textbf{视觉识别(VI)图像设置}\\
...
\begin{frame}{\pkg{beamethemescu} 宏包已废除参数}
	\begin{table}[h]
		...
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.3\textwidth}p{0.15\textwidth}p{0.45\textwidth}}
			\textbf{\Arg{key}} & \textbf{废除版本} & \textbf{说明}\\
			\midrule

			~\alert{\texttt{Miniframes}}\texttt{ =  negate} & v2.1b & 由 \texttt{Miniframes}\texttt{ = none} 替代.\\
			~\alert{\texttt{NavigationTool}}\texttt{  =  negate} & v2.1b & 由 \texttt{NavigationTool}\texttt{ = none} 替代.\\
			~\alert{\texttt{Background}}\texttt{ =  true} & v2.1c & 由 \texttt{Background}\texttt{ = SCU-Full} 替代.\\
			~\alert{\texttt{Background}}\texttt{ =  false} & v2.1c & 由 \texttt{Background}\texttt{ = SCU-Lite} 替代.\\
		\end{tabular}
	\end{table}
\end{frame}
