...
% Version: 2025/10/06 v2.1b (Original Version: 2021/11/30 v1.0a).
...
% ----------------
\usetheme[
	% MathFont=LM, % LM(默认) | XITS → 数学字体设置
	% ColorDisplay=JXred, % JXred(默认) | BSblue | Custom → 主题色显示设置
	CodeDisplay=minted, % listing(默认) | minted → 代码高亮显示设置
	MintedStyle=lightmode, % lightmode(默认) | darkmode | ⟨custom⟩ → minted 样式设置 (需优先设置 CodeDisplay=minted)
	% BlockDisplay=colorful, % colorful(默认) | followtheme | allgrey → 区块颜色显示设置
	% LanguageMode=cn, % cn(默认) | en → 语言模式设置
	Miniframes=follow, % follow(默认) | separate | none → 页眉小节迷你帧设置
	% NavigationTool=1-2-3, % ⟨参考 Manual 设置⟩ | none → 页脚导航工具栏设置
	% BIBMode=biber, % biber(默认) | none → 参考文献引擎设置
	% BIBStyle=biber-gb7714, %  biber-gb7714(默认) → 参考文献样式设置(设置 BIBMode=none 时无效)
	% ContentMuticols=true, % true(默认) | false → 目录帧双栏显示设置
	Background=true, % true(默认) | false → 背景显示设置
]{scu}
...
% ----------------
\title[四川大学Beamer模板 | User's Manual Rev~v2.1b]{四川大学Beamer模板}
\subtitle{用户手册~~~~Rev v2.1b} % subtitle 未设置页脚显示项, 请在 title 中设置.
