...
% Version: 2026/06/26 v2.1g (Original Version: 2021/11/30 v1.0a).
...
\usepackage{tikz}
\usetikzlibrary{shapes.geometric,positioning,shadings,arrows,calc,decorations.pathmorphing,patterns}
\usepackage{array}
\usepackage{cancel}
...
% ----------------
\title[四川大学Beamer模板 | User's Manual Rev~v2.1g]{四川大学Beamer模板}
\subtitle{用户手册~~~~Rev v2.1g} % subtitle 未设置页脚显示项, 请在 title 中设置.
...
\institute{%
	...
	\vspace*{-6pt} \and
	\inst{a} \mailscu{MaLJFake@taichi.hunyuan} ~ \inst{b} \mailscu{MaXJFake@scu.edu.cn}
}
\date{2026年6月26日}
...
\begin{frame}{快速开始}
	详细操作步骤及完整 MWE 请参见 \href{https://github.com/FvNCCR228/SCU-Beamer-Theme\#快速开始}{\color{scublue}README · 快速开始} 部分.\\

	\begin{tikzpicture}[
		node distance=0.3cm and 0.4cm,
		startstop/.style={rectangle, rounded corners, minimum width=2cm,
			minimum height=.7cm, text width=1.8cm, align=center,
			draw=scured, fill=scured10},
		process/.style={rectangle, minimum width=2cm, minimum height=.7cm,
			text width=1.8cm, align=center, draw=scugrey, fill=scugrey10},
		decision/.style={diamond, inner sep=2pt, minimum width=1.5cm,
			minimum height=.7cm, align=center,
			draw=scublue, fill=scublue10, aspect=2.0},
		arrow/.style={->, >=stealth, thick, color=scugrey},
	]
		% 主流程: start → create (↑) → decide (→)
		\node (start)   [startstop]  {从Github/Gitee克隆或下载};
		\node [anchor=north west, align=left, xshift=-.5cm, yshift=-.1cm,
			at=(start.south west), color=scugrey]
			{(1) 克隆: \\\phantom{(1)}\texttt{git clone} \texttt{https://github.com/} \\
			\phantom{(1)}\texttt{FvNCCR228/SCU-Beamer-Theme.git} \\[.5ex] (2) 从 Releases 页面下载};
		\node (create)  [process, above=1 of start]
			{创建主 \texttt{.tex} 文件 (假设为 \texttt{exp.tex})};
		\node (decide)  [decision, right=.9cm of create]
			{如何创建?};

		% 选项堆叠 (从 decide 右侧引出, 中心在同一垂直线, 间距 2:6)
		\node (ref)  [process, minimum width=2.8cm, text width=2.6cm] at ($(decide.center)+(3,1.2)$)
			{参考仓库中 \texttt{main.tex} 或 \texttt{main-en.tex} 示例文件};
		\node (mwe)  [process, minimum width=2.8cm, text width=2.6cm] at ($(decide.center)+(3,-3.6)$)
			{展开 README 中的 MWE 保存为 \texttt{exp.tex} 或按需修改};

		% 导入章节文件 (位于 ref 与 mwe 之间, 中心在同一垂直线)
		\node (sec)  [process, minimum width=2.8cm, text width=2.6cm] at ($(ref.center)!0.5!(mwe.center)$)
			{在 \texttt{include-sec/} 下创建各章节 \texttt{.tex} 文件};

		% 后半流程 (与 sec 同 y)
		\node (compile) [process, right=.9cm of mwe]
			{编译 \texttt{latexmk exp.tex}};
		\node (end)     [startstop, right=.9cm of compile]
			{编译完成!\\ 查看生成的 PDF};

		% 清理命令
		\node (cleanc) [process] at ($(end.north)+(0,1)$)
			{清除中间文件 \\ \texttt{latexmk -c}\\ 或 \texttt{latexmk -c exp.tex}};
		\node (cleanC) [process] at ($(cleanc.north)+(0,1.6)$)
			{清除全部生成文件 \\ \texttt{latexmk -C}\\ 或 \texttt{latexmk -C exp.tex}};

		% 箭头
		\draw [arrow] (start)  -- (create);
		\draw [arrow] (create) -- (decide);
		\draw [arrow] (compile) -- (end);
		\draw [arrow] (end) -- (cleanc);
		\draw [arrow] (cleanc) -- (cleanC);
		% decide → 选项
		\draw [arrow] (decide.north) |- (ref.west) node[pos=.25, color=scured, xshift=.2cm] {(2)};
		\draw [arrow] (decide.south) |- (mwe.west) node[pos=.25, color=scured, xshift=.2cm] {(1)};
		% 导入 ↔ 选项 (sec 上连 ref, 下连 mwe)
		\draw [arrow] (sec.north) -- (ref.south) node[midway, xshift=.4cm]
			{通过 \cmd{include} 或 \cmd{input} 导入};
		\draw [arrow] (sec.south) -- (mwe.north) node[midway, xshift=.4cm]
			{通过 \cmd{include} 或 \cmd{input} 导入};
		% 选项 → latexmk 编译
		\draw [arrow] (ref.east) -| (compile.north);
		\draw [arrow] (mwe.east) -- (compile);
		% decide → sec (完整链路)
		\draw [arrow] (decide) |- (sec);

		% 侧注
		\node [right=0.15cm of sec, font=\itshape, align=left]
			{\texttt{\upshape include-sec/} 为预设 \\
			目录, 且 \texttt{\upshape tmp/build/} \\
			下已预建对应子目录 \\ (latexmk 安全限制)};
	\end{tikzpicture}
\end{frame}
...
% -------- SEC: 页面布局 --------
\include{manual-sec/basic-layout}

% -------- SEC: 内容排版 --------
\include{manual-sec/basic-content}
\include{manual-sec/basic-content-2}

% -------- SEC: 进阶使用 --------
\include{manual-sec/advanced-usage}
